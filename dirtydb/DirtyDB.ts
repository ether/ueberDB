...
export class Dirty extends EventEmitter {
     ...
     */
    set(key:string, val:string|undefined|{}, cb:()=>{}) {
        if (val === undefined) {
     ...
     */
    update(key:string, updater:(v:string)=>{}, cb:()=>{}) {
    ...
    _flush() {
        ...
        for (const [key, cbs] of this._queue) {
            ...
            this._waitForDrain = !this._writeStream.write(data, (err: CustomError) => {
...
export default new Proxy(Dirty, {apply: (target, thisArg, args) => new Dirty(args.toString())});
