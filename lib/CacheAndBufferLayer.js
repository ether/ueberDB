...
/**
This module is made for the case, you want to use a SQL-Based Databse or a
KeyValue Database that can only save strings(and no objects), as a JSON KeyValue Store.

The idea of the dbWrapper is to provide following features:

* automatic JSON serialize/deserialize to abstract that away from the database
driver and the module user.
* cache reads. A amount of KeyValues are hold in the memory, so that reading is faster.
* Buffer DB Writings. Sets and deletes should be buffered to make them in a
setted interval with a bulk. This reduces the overhead of database transactions
and makes the database faster. But there is also a danger to loose data integrity,
to keep that, we should provide a flush function.

All Features can be disabled or configured. The Wrapper provides default settings
that can be overwriden by the driver and by the module user.
*/
...
const defaultSettings =
{
  ...
  cache: 1000,
  // the interval in ms the wrapper writes to the database.
  // To Disable interval writes just set it to zero
  writeInterval: 100,
  ...
  if (operations.length > 0) {
    ...
    db.wrappedDB.doBulk(operations, (err) => {
      ...
      for (const i in callbacks) {
        if (callbacks[i]) callbacks[i](err);
      }
...
exports.database = function (wrappedDB, settings, logger) {
  ...
  // set the flushing flag to false, this flag shows that there is a
  // flushing action happing at the moment
...
exports.database.prototype.get = function (key, callback) {
  ...
  if ((this.settings.cache > 0) && this.buffer[key]) {
    ...
    // caching is disabled but its still in a dirty writing cache,
    // so we have to get the value out of the cache too
    this.logger.
        debug(`GET    - ${key} - ${JSON.stringify(this.buffer[key].value)} - from dirty buffer`);
    this.buffer[key].timestamp = new Date().getTime();
    callback(null, this.buffer[key].value);
  } else {
    // get it direct
...
exports.database.prototype.set = function (key, value, bufferCallback, writeCallback) {
  ...
  if (this.settings.writeInterval > 0) {
    ...
    // we need this as an array, cause the value may be many times overwritten
    // bevors its finally written to the database, but all callbacks must be called
...
/**
 ...
 @param sub is a array, for example if you want to access
 object.test.bla, the array is ["test", "bla"]
*/
...
exports.database.prototype.gc = function () {
  // If the buffer size is under the settings size or cache is disabled ->
  // return cause there is nothing to do
